...
import colin.sort.Heap;

public class PrintHeap {


	// 以树状打印整个堆结构
	public static void treePrint(Heap h){
		int data[]=h.data;
		if( h.size() >= 100 ){
		...
		System.out.println("The max heap size is: " + h.size());
		...
		for( int level = 0 ; level < maxLevel ; level ++ ){
			...
			int curLevelNumber = Math.min(h.size()-(int)Math.pow(2,level)+1,(int)Math.pow(2,level));
			boolean isLeft = true;
